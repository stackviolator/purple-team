...
class AtomicTest:
    ...
    def write_log_error(self, api, name, guid, desc, platforms, timeout, callback_id, err_str):
        data = [
            {
                'command':"None", 
                'timestamp':"None",
    ...
    def strip_args(self, cmd):
        ...
        for arg in arguments:
            ...
            cmd = cmd.replace(arg, str(self.args.get(a).get('default')))
        return cmd
    ...
    async def check_prereqs(self):
        ...
        for d in self.dependencies:
            ...
            if "Test Passed" not in output:
                ...
                if "Test Passed" not in output:
                    err_str = f"[-] Failed to satisfy prerequisites for {self.name}\n\tget_prereq_command: {cmd}"
                    print(err_str)
                    # Write error into CSV
                    self.write_log_error(self.api, self.name, self.guid, self.description, self.platforms, self.timeout, self.callback_id, err_str)
    ...
    async def run_executor(self):
        ...
        err_str = f"[-] Failed getting command"
        if cmd is None:
            self.write_log_error(self.api, self.name, self.guid, self.description, self.platforms, self.timeout, self.callback_id, err_str)
            cmd = ""
