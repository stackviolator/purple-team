...
class Atomic:
    ...
    def parse_yaml(self):
        ...
        self.tests = [
            AtomicTest(dict(x), self.api_instance, self.timeout, self.callback_id)
            for x in self.yaml["atomic_tests"]
        ]  # sick list comprehension
...
class AtomicTest:
    ...
    async def check_prereqs(self, special_exec, method):
        ...
        if not has_elevation:
            ...
            self.api_instance.log_error(
                self.name,
                self.guid,
                self.description,
                self.platforms,
                self.timeout,
                self.api_instance.child_callback_id,
                err_str,
            )
        ...
        if self.executor["name"].lower() in DISALLOWED_EXECUTORS:
            ...
            raise Exception(
                f"Unsupported executor \"{self.executor['name']}\" for task {self.name}"
            )
        ...
        if special_exec:
            ...
            if method == "pe" or method == "dotnet":
                ...
                if binpath == "":
                    ...
                    sys.exit(0)
                # Register file
                await self.api_instance.register_file(
                    binpath, self.api_instance.child_callback_id
                )
            ...
            for d in self.dependencies:
                ...
                if "Test Passed" not in output:
                    print(
                        f"[-] Prereqs not met for test: {self.name}, running get_prereq_command "
                    )
                    # Run get_prereq_command
                    get_prereq_cmd = Command(
                        self.dependency_executor,
                    ...
                    if output is None:
                        ...
                        if output is None:
                            raise Exception(
                                f"get_prereq_command task execution timed out"
                            )

                    if "Test Passed" not in output:
                        raise Exception(
                            f"Failed to satisfy prerequisites for {self.name}\n\tget_prereq_command: {prereq_cmd.parameters}"
                        )
